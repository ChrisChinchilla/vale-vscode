...
- [x] **Resolve remaining dependency findings.** Stale: `npm audit` and `npm audit --omit=dev` both now report 0 vulnerabilities (verified 2026-09-16). The dependency-cleanup work already tracked elsewhere in this file (removing TSLint/Mocha/`vscode-test`, upgrading `vscode-languageclient`) had already resolved these; this checkbox was just never updated after the fact.
...
- [x] **Harden GitHub Actions.** Every `uses:` across all eight workflows now pins a full commit SHA (resolved via `gh api repos/<owner>/<repo>/commits/<tag>`), with a trailing `# vX` comment so Dependabot/Renovate can still track updates. Also bumped the two remaining `actions/checkout@v3` instances (`preRelease.yml`, `release.yml`) to v4 for consistency with the rest. Added `permissions:` to every job that was missing one and previously fell back to the repo's default token scope: `build.yaml`'s `build` job (`contents: read`), `publishPreRelease.yml`/`publishTags.yml`'s `deploy` jobs (`contents: read`), `versionParity.yml` (workflow-level `permissions: {}` - it never checks out the repo or calls the API), and `todo-issue.yml` (`contents: read`, `issues: write`). Verified with `actionlint` (0 findings) - no functional/behavioral change intended beyond the pins and scoping. While fixing `todo-issue.yml`'s permissions, found it was also missing the `actions/checkout` step `alstr/todo-to-issue-action` requires to scan the repo (confirmed via its own docs) - added it, since a workflow that couldn't have been working is arguably a bigger problem than the one being fixed.
...
- [x] **Handle unsupported Linux libc explicitly.** Alpine/musl is detected before download and receives an actionable glibc-only error. **Regressed in v1.2.0** ([issue #123](https://github.com/ChrisChinchilla/vale-vscode/issues/123)): the original check read `process.report` directly, which is unavailable inside the VS Code extension host, so it always treated real glibc hosts as musl and refused to activate. Fixed by detecting via `detect-libc` (falls back to filesystem-based checks that don't need `process.report`) and, more importantly, failing open on an undetermined libc family instead of treating "unknown" as "musl". Pure unit coverage verifies Linux glibc, Linux musl, Linux undetermined (regression case), and non-Linux behavior; a real-extension-host assertion in `test/integration/suite/index.ts` (CI's `ubuntu-latest` job) checks that the production `ensureLanguageServerBinary()` accepts a prepared cached binary there, since the plain-Node unit tests structurally can't see this class of bug. See `.claude/notes/glibc-detection-fix.md`.

- [x] **Make remote startup diagnosable.** The Vale output channel logs extension-host location, platform/libc, trust, workspace/config paths, selected Vale execution mode/path, vale-ls version/path, installation failures, and client startup errors. **Vale: Show Diagnostics** and **Vale: Restart Language Server** remain available even when initial server installation/activation fails - specifically, failures inside `startLanguageServer()`. See the item below for the gap that left activation itself capable of failing silently before this held.

- [x] **Fix silent activation failure with no registered commands.** [Issue #129](https://github.com/ChrisChinchilla/vale-vscode/issues/129): reported as vale-vscode 1.2.1 doing nothing at all - no errors, no linting, no `vale` process, an empty output channel even with verbose tracing, and every `vale.*` command reporting "command not found". Root cause: `activate()` (`src/lifecycle.ts`) only wrapped `startLanguageServer()` in try/catch; the readability-surfaces setup, the `detect-libc` calls added for the #123 fix, and the active-client bookkeeping all ran unguarded *before* `registerCommands()`/`registerCodeActions()`, so an exception anywhere in that chain silently rejected the whole `activate()` promise with nothing registered and nothing written to the output channel. We could not reproduce the reporter's exact throw against their environment (Fedora 44, VS Code installed system-wide), so this fixes the structural gap rather than a confirmed single trigger. Fixed by registering commands/code actions immediately after creating the output channel, before any of the startup diagnostics, and wrapping that diagnostics block in its own try/catch so a failure there is surfaced (like `startLanguageServer()` failures already were) instead of aborting activation. `test/integration/suite/index.ts` now asserts every declared command is registered after activation. See `.claude/notes/silent-activation-failure-fix.md`.
...
- [x] **Fix executable paths containing spaces.** Paths were incorrectly escaped even though no shell is used for the language server (`valePath.replace(/(\s)/, "\\ ")` - only the first space, and shell-style escaping is meaningless for a no-shell `spawn`, so it corrupted otherwise-valid paths instead of fixing anything). Already gone by the time of `cc6509c` (part of the LSP rewrite/module-split work) - `ensureLanguageServerBinary` (`src/languageServer.ts`) now returns the literal, unescaped path, with a comment explaining why. Confirmed as the root cause of the `ENOENT`/backslash-in-path reports on [issue #80](https://github.com/ChrisChinchilla/vale-vscode/issues/80) (e.g. `c:\Users\Eddie\ McHam\...`) - those reports were all against versions predating this fix (v0.30.0 and earlier); not reproducible on current `main`. This checkbox was simply never updated after the fix landed.

- [x] **Apply the custom filter setting.** `vale.valeCLI.filter` is now read in `buildValeConfig` (`src/config.ts`) and combined with the legacy alert-level/spelling filters via `buildValeFilterExpression` (`src/utils.ts`), AND-ing a parenthesized custom expression onto whatever the legacy settings produce. Added to `VALE_CONFIG_SETTINGS` in `src/languageServer.ts` so changing it restarts the affected client like the other filter-affecting settings.
...
- [x] **Pass `vale.valeCLI.config` to direct CLI commands.** Confirmed as the cause of [issue #100](https://github.com/ChrisChinchilla/vale-vscode/issues/100): **Vale: Sync**/**Show Configuration**/**Show Readability Metrics** and the vocabulary lookups (`getStylesPathsFromVale`) never passed `--config`, relying entirely on Vale's own ancestor-search discovery from the workspace root - which can't find a config file in a subdirectory (linting worked regardless, since vale-ls receives `configPath` separately). Reproduced directly: without `--config`, `vale sync` silently found and synced an unrelated pre-existing global config instead of failing loudly. Fixed via `buildValeConfigArgs` (`utils.ts`) and `resolveCommandConfigPath` (`commands.ts`), applied to every direct CLI invocation. The report's "ANSI codes in the sync failure notification" half was already fixed as a side effect of the CLI-execution rewrite (the current `runValeCommand` never embeds raw command output in the shown `Error`). See `.claude/notes/direct-cli-config-path.md`.

- [x] **Fix vocabulary path selection.** `getStylesPathsFromVale` (`src/cli.ts`) already returns a single resolved path, not an array - the vestigial `findVocabStylesPath` helper in `src/vocabulary.ts` iterated over it as if it were a list of candidates, and its fallback (`stylesPaths[0]`) sliced the first *character* off that path string. Since both of that helper's branches always returned the same value, it was a no-op wrapped around a bug; removed it and use the resolved path directly.

- [x] **Validate vocabulary input.** `addToVocabulary` (`src/vocabulary.ts`) now rejects a vocabulary name containing a path separator or leading/trailing whitespace, and independently checks that the resolved vocabulary directory stays inside `<StylesPath>/config/vocabularies` (catches a bare `..` component, which contains no separator). Words are passed through the new `sanitizeVocabularyWord` (`src/utils.ts`), which collapses embedded newlines/carriage returns so a multiline editor selection can't inject extra lines into `accept.txt`/`reject.txt`.
...
- [x] **Remove or implement no-op settings.** All three now do something (owner's call: implement rather than remove):

  - `vale.maxNumberOfProblems` - applied via `handleDiagnostics` client middleware in `src/languageServer.ts` (`capDiagnostics`, `src/utils.ts`), since vale-ls has no such limit server-side.
  - `vale.doNotShowWarningForFileToBeSavedBeforeLinting` - **Vale: Show Readability Metrics** reads the file from disk, so `confirmSavedBeforeLinting` (`src/commands.ts`) now warns and offers to save first on a dirty document, gated by this setting.
...
- [x] Remove duplicated accept/reject handlers and centralize error formatting and workspace resolution. **Add to Accept List**/**Add to Reject List** now share one handler (`addSelectionToVocabulary`, parameterized by target file) instead of two ~55-line copies. Added `resolveCommandContext` (`src/commands.ts`) as the single place that resolves the workspace folder, settings, execution mode, and config path together, scoped consistently - every command (vocabulary add, Sync, Show Configuration, Show Readability Metrics) now goes through it instead of repeating the same five-line sequence. Added `showCommandError`/`formatError` so every command reports a caught error the same way (`Vale: <verb> - <message>`) instead of five slightly different inline ternaries.
...
- [x] Implement the diagnostic limit through client middleware so the exposed setting actually works. `handleDiagnostics` middleware + `capDiagnostics` (`src/utils.ts`) - see the "Remove or implement no-op settings" functional-defects item above.
...
- [x] Resolve commands relative to the active document’s workspace folder, prompting only when the choice is ambiguous. `getRelevantWorkspaceFolder` (`src/workspaceFolders.ts`) already inferred the folder from the active editor; it now also prompts via `vscode.window.showWorkspaceFolderPick` when there's no active-editor folder to infer from *and* more than one workspace folder exists, instead of silently defaulting to the first. `resolveCommandContext` treats a cancelled prompt as "abort the command," not "fall back to a guess."
- [x] Raise the TypeScript target from ES2017 to a modern Node target and remove unnecessary casts and boxed `String` types. `tsconfig.json` now targets ES2022 (matching `tsconfig.test.json`, and well within `engines.node`'s `^22.13.0 || >=24.0.0` floor). Removed `config.ts`'s `valeArgs`/`valeConfigOptions` types, which existed only to justify `as unknown as valeArgs` casts on plain strings/booleans - `buildValeConfig` now returns a properly-typed `ValeInitializationOptions` interface with no casts at all, and the `String(valeConfig.filter)`/`String(valeConfig.configPath)` call sites in `languageServer.ts` (redundant now that those fields are provably `string`) were simplified too.
...
- Tests: `npm test` runs `src/utils.test.ts` (pure, `vscode`-free helpers) via `tsc -p tsconfig.test.json` + Node's built-in test runner (101 tests). No module importing `vscode` at load time has direct test coverage — that needs the extension host (or a mock) to test directly; see `src/*.ts` module split in `.claude/notes/module-split.md` for where such logic could be extracted from if this becomes worth doing.
